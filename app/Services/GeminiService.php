...
EOT;
    ...
    public function uploadFileToGemini(string $filePath, string $mimeType = 'video/mp4', string $displayName = 'video'): array
    {
        ...
        try {
            ...
            $initResponse = $initClient->request('POST', 'https://generativelanguage.googleapis.com/upload/v1beta/files', [
                'query' => ['key' => $this->apiKey],
                'headers' => [
                    'X-Goog-Upload-Protocol' => 'resumable',
                    'X-Goog-Upload-Command' => 'start',
            ...
            $uploadUrl = $initResponse->getHeaderLine('X-Goog-Upload-URL');
            if (empty($uploadUrl)) {
                return ['success' => false, 'error' => 'No upload URL returned from Gemini File API'];
            }

            // Step 2: Upload the binary data
            $uploadResponse = $initClient->request('POST', $uploadUrl, [
                'headers' => [
            ...
            if (empty($fileUri)) {
                \Log::error('[GeminiService] Upload succeeded but no file URI', ['body' => $body]);
                return ['success' => false, 'error' => 'Upload succeeded but no file URI returned'];
            }
            ...
            \Log::error('[GeminiService] File upload failed', ['error' => $e->getMessage()]);
            return ['success' => false, 'error' => 'Gemini file upload failed: ' . $e->getMessage()];
        }
    }

    /**
     * Analyze an uploaded video using Gemini's native video understanding.
     * Uses fileData (from File API upload) instead of inlineData.
     */
    public function analyzeVideoWithPrompt(string $fileUri, string $prompt, array $options = []): array
    {
        $model = $options['model'] ?? 'gemini-2.5-flash';
        ...
        try {
            $body = $this->sendGenerateContentRequest($model, $payload, [], 120);
            $text = $body['candidates'][0]['content']['parts'][0]['text'] ?? '';
