...
must_haves:
  truths:
    ...
    - "validateContinuity() method returns array of issues for voice mismatches"
  artifacts:
...
Output: `VoiceRegistryService.php` with initialization, registration, lookup, validation, and continuity methods.
</objective>
...
- `array $mismatchHistory = []` - tracks detected mismatches for validateContinuity()
...
2. `registerCharacterVoice(string $characterName, string $voiceId, string $source): bool` (protected)
   ...
   - If already in `$characterVoices`:
     ...
     - If voiceId differs: Log::warning with mismatch details, add to `$mismatchHistory`, return false
...
7. `validateContinuity(): array`
   - Returns array of issues detected in the registry
   - Check for mismatches stored in `$mismatchHistory` during registration
   - Return structure (matching Phase 16 pattern):
     ```php
     return [
     ...
     ```
   - Each issue in array contains: character, expectedVoice, actualVoice, source
...
Grep: `grep -c "function" modules/AppVideoWizard/app/Services/VoiceRegistryService.php` - returns 7 (7 methods)
Grep: `grep "class VoiceRegistryService" modules/AppVideoWizard/app/Services/VoiceRegistryService.php` - class exists
Grep: `grep "validateContinuity" modules/AppVideoWizard/app/Services/VoiceRegistryService.php` - method exists
  </verify>
  <done>
VoiceRegistryService.php exists with all 7 methods including validateContinuity(), handles both voice formats, logs appropriately, no syntax errors
  </done>
</task>
...
Grep: `grep "validateContinuity returns issues" modules/AppVideoWizard/app/Services/VoiceRegistryService.php` - test scenario 7 documented
  </verify>
  <done>
Test scenarios documented in class, covering all key behaviors including validateContinuity()
  </done>
</task>
...
3. All methods present: grep for each method name (7 methods total)
...
6. validateContinuity exists: grep for `function validateContinuity`
</verification>

<success_criteria>
- VoiceRegistryService.php created in Services directory
- Class has all 7 methods with correct signatures
- validateContinuity() method returns issues array with valid/issues/statistics structure
...
- Mismatch detection logs warnings and tracks in mismatchHistory
- Test scenarios documented as comments (including validateContinuity tests)
