    ...
    <div class="aith-tool">
        ...
        <div class="aith-card">
            <div class="aith-e-tool-header">
                <div class="aith-e-tool-icon" style="background:linear-gradient(135deg,#8b5cf6,#3b82f6);">
                    <i class="fa-light fa-chart-mixed" style="color:#fff;font-size:1.1rem;"></i>
                </div>
                <div class="aith-e-tool-info">
                    <h2>Cross-Platform FB Arbitrage</h2>
                    <p>Find content gaps between YouTube and Facebook</p>
                </div>
            ...
            <div class="aith-form-group">
                <label class="aith-label">Facebook Niche Focus (optional)</label>
                <input type="text" wire:model="fbNiche" class="aith-input"
                       placeholder="e.g. fitness, beauty, tech reviews">
                @error('fbNiche') <span class="aith-e-field-error">{{ $message }}</span> @enderror
            </div>
            <button wire:click="analyze" wire:loading.attr="disabled" class="aith-btn-primary" style="width:100%;margin-top:1rem;">
                <span wire:loading.remove wire:target="analyze">
                    <i class="fa-light fa-chart-mixed"></i> Analyze Arbitrage
                </span>
            ...
            <div class="aith-e-result-header">
                <span class="aith-e-result-title">FB Arbitrage Analysis</span>
                <div class="aith-e-result-actions">
                    <button onclick="enterprisePdfExport('pdf-content-fb-yt-arbitrage', 'FB-Arbitrage')" class="aith-e-btn-pdf">
            ...
            <div id="pdf-content-fb-yt-arbitrage">
                ...
                <div style="display:flex;gap:1rem;align-items:flex-start;flex-wrap:wrap;">
                    ...
                    <div style="flex:1;min-width:200px;">
                        ...
                        <div style="display:flex;gap:1rem;flex-wrap:wrap;">
                            ...
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.6);"><i class="fa-light fa-video" style="margin-right:0.25rem;"></i> {{ $yt['video_count'] ?? '0' }} videos</span>
                        </div>
                        <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-top:0.375rem;">
                            ...
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.6);"><i class="fa-light fa-thumbs-up" style="margin-right:0.25rem;"></i> {{ $yt['avg_likes'] ?? '0' }} avg likes</span>
            ...
            <div class="aith-e-section-card">
                ...
                <div style="overflow-x:auto;">
                    <table class="aith-e-table">
                        <thead><tr><th>Topic</th><th>YT Performance</th><th>FB Saturation</th><th>Opportunity</th><th>Best FB Format</th><th>Reasoning</th></tr></thead>
                        <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @php $sat = strtolower($gap['fb_saturation'] ?? ''); @endphp
                                <span class="aith-e-tag {{ $sat === 'low' ? 'aith-e-tag-high' : ($sat === 'medium' ? 'aith-e-tag-medium' : 'aith-e-tag-low') }}">{{ $gap['fb_saturation'] ?? '' }}</span>
                            </td>
                            <td>
                            ...
                            <td><span style="font-size:0.75rem;padding:0.125rem 0.375rem;border-radius:0.25rem;background:rgba(59,130,246,0.1);color:#3b82f6;">{{ $gap['best_fb_format'] ?? '' }}</span></td>
            ...
            <div class="aith-e-section-card">
                ...
                <div style="overflow-x:auto;">
                    <table class="aith-e-table">
                        <thead><tr><th>YouTube Type</th><th>FB Format</th><th>Adaptation Tips</th></tr></thead>
                        <tbody>
                        @foreach($result['format_recommendations'] as $rec)
                        <tr>
                            <td style="font-size:0.85rem;color:rgba(255,255,255,0.6);">{{ $rec['youtube_type'] ?? '' }}</td>
                            <td><span style="font-size:0.75rem;padding:0.125rem 0.375rem;border-radius:0.25rem;background:rgba(59,130,246,0.1);color:#3b82f6;">{{ $rec['fb_format'] ?? '' }}</span></td>
                ...
                <div style="display:flex;align-items:flex-start;gap:0.75rem;padding:0.5rem 0;border-bottom:1px solid rgba(255,255,255,0.06);">
                    ...
                    <div style="flex:1;">
                        ...
                        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-top:0.25rem;">
                            <span style="font-size:0.75rem;color:rgba(255,255,255,0.4);">{{ $win['expected_result'] ?? '' }}</span>
                            @if(isset($win['timeframe']))
                            <span style="font-size:0.75rem;color:rgba(255,255,255,0.4);">· {{ $win['timeframe'] }}</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            {{-- Next Steps --}}
            @php
                $nextSteps = config('appaitools.enterprise_tools.fb-yt-arbitrage.next_steps', []);
