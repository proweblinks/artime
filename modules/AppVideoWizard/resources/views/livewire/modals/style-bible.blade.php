...
<div x-data="{ isOpen: false }"
    ...
    <div class="vw-modal"
        ...
        <div style="padding: 0.5rem 1rem; border-bottom: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            <div>
                <h3 style="margin: 0; color: var(--vw-text); font-size: 1rem; font-weight: 600;">🎨 {{ __('Style Bible') }}</h3>
                <p style="margin: 0.15rem 0 0 0; color: var(--vw-text-secondary); font-size: 0.7rem;">{{ __('Define your visual DNA for consistent imagery') }}</p>
            </div>
        ...
        <div style="flex: 1; overflow-y: auto; padding: 0.75rem 1rem;">
            ...
            <div style="margin-bottom: 0.75rem;">
                <label style="display: block; color: var(--vw-text); font-size: 0.65rem; margin-bottom: 0.3rem;">{{ __('Reference Image') }}</label>
                <div style="display: flex; gap: 0.75rem;">
                    ...
                    <div style="width: 150px; flex-shrink: 0;">
                        @if(!empty($sceneMemory['styleBible']['referenceImage']))
                            <div style="position: relative;">
                                <img src="{{ $sceneMemory['styleBible']['referenceImage'] }}"
                                     style="width: 100%; aspect-ratio: 16/9; object-fit: cover; border-radius: 0.35rem; border: 1px solid var(--vw-border);">
                                {{-- Source indicator --}}
                                <div style="position: absolute; top: 0.2rem; left: 0.2rem; background: rgba(0,0,0,0.7); padding: 0.1rem 0.3rem; border-radius: 0.2rem; font-size: 0.45rem; color: var(--vw-text);">
                        ...
                        @else
                            <div style="width: 100%; aspect-ratio: 16/9; background: rgba(0,0,0,0.03); border: 2px dashed var(--vw-border); border-radius: 0.35rem; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                                <span style="font-size: 1.25rem; color: var(--vw-text-secondary);">🖼️</span>
                                <span style="font-size: 0.55rem; color: var(--vw-text-secondary); margin-top: 0.15rem;">{{ __('No reference yet') }}</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div style="flex: 1; display: flex; flex-direction: column; gap: 0.35rem;">
                        ...
                        <div x-data="{ uploading: false }" style="position: relative;">
                            ...
                            <button type="button"
                                    style="width: 100%; padding: 0.4rem 0.75rem; background: var(--vw-border); border: 1px solid var(--vw-border); border-radius: 0.35rem; color: var(--vw-text); font-size: 0.65rem; cursor: pointer; font-weight: 500;">
                        ...
                        <p style="color: var(--vw-text-secondary); font-size: 0.55rem; margin: 0;">
            ...
            {{-- Quick Templates --}}
            <div style="margin-bottom: 0.75rem;">
                <label style="display: block; color: var(--vw-text); font-size: 0.65rem; margin-bottom: 0.3rem;">{{ __('Quick Templates') }}</label>
                <div style="display: flex; gap: 0.3rem; flex-wrap: wrap;">
                    <button type="button" wire:click="applyStyleTemplate('cinematic')" style="padding: 0.25rem 0.5rem; background: rgba(var(--vw-primary-rgb), 0.06); border: 1px solid rgba(var(--vw-primary-rgb), 0.12); border-radius: 0.25rem; color: var(--vw-text-secondary); font-size: 0.6rem; cursor: pointer;">🎬 {{ __('Cinematic') }}</button>
                    <button type="button" wire:click="applyStyleTemplate('documentary')" style="padding: 0.25rem 0.5rem; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); border-radius: 0.25rem; color: #16a34a; font-size: 0.6rem; cursor: pointer;">🎥 {{ __('Documentary') }}</button>
                    <button type="button" wire:click="applyStyleTemplate('anime')" style="padding: 0.25rem 0.5rem; background: rgba(236,72,153,0.15); border: 1px solid rgba(236,72,153,0.3); border-radius: 0.25rem; color: #be185d; font-size: 0.6rem; cursor: pointer;">🎌 {{ __('Anime') }}</button>
                    <button type="button" wire:click="applyStyleTemplate('noir')" style="padding: 0.25rem 0.5rem; background: rgba(0,0,0,0.04); border: 1px solid var(--vw-border); border-radius: 0.25rem; color: var(--vw-text); font-size: 0.6rem; cursor: pointer;">🖤 {{ __('Film Noir') }}</button>
                    ...
                    <button type="button" wire:click="applyStyleTemplate('photorealistic')" style="padding: 0.25rem 0.5rem; background: rgba(6,182,212,0.15); border: 1px solid rgba(6,182,212,0.3); border-radius: 0.25rem; color: #0891b2; font-size: 0.6rem; cursor: pointer;">📷 {{ __('Photorealistic') }}</button>
                </div>
            </div>
            ...
            <div x-data="{ open: false }" style="margin-bottom: 0.5rem; border: 1px solid var(--vw-border); border-radius: 0.35rem; overflow: hidden;">
                <button type="button" @click="open = !open" style="width: 100%; padding: 0.4rem 0.5rem; background: rgba(0,0,0,0.03); border: none; display: flex; justify-content: space-between; align-items: center; cursor: pointer;">
            ...
            {{-- Negative Prompt --}}
            <div style="margin-bottom: 0.5rem;">
                <label style="display: block; color: var(--vw-text); font-size: 0.6rem; margin-bottom: 0.2rem;">{{ __('Negative Prompt (Things to Avoid)') }}</label>
                <textarea wire:model.blur="sceneMemory.styleBible.negativePrompt"
                          ...
                          style="width: 100%; padding: 0.4rem 0.5rem; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); border-radius: 0.35rem; color: #dc2626; font-size: 0.7rem; min-height: 40px; resize: vertical;"></textarea>
                <p style="color: var(--vw-text-secondary); font-size: 0.5rem; margin: 0.2rem 0 0 0;">
        ...
        <div style="padding: 0.5rem 1rem; border-top: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            <label style="display: flex; align-items: center; gap: 0.4rem; color: var(--vw-text); font-size: 0.7rem; cursor: pointer;">
                <input type="checkbox" wire:model.live="sceneMemory.styleBible.enabled" style="accent-color: var(--vw-primary);">
