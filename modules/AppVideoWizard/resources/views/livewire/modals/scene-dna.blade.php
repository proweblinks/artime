...
<div class="vw-modal-overlay"
    ...
    <div class="vw-modal"
        ...
        @endphp
        <div style="padding: 0.5rem 1rem; border-bottom: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; background: linear-gradient(90deg, rgba(6,182,212,0.1), rgba(var(--vw-primary-rgb), 0.04));">
            <div>
                <h3 style="margin: 0; color: var(--vw-text); font-size: 1rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem;">
                    <span style="background: linear-gradient(135deg, #06b6d4, var(--vw-primary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
                        {{ __('Scene DNA') }}
                    </span>
                    <span style="font-size: 0.6rem; color: #10b981;">&#x2713; {{ __('Auto-sync') }}</span>
                </h3>
                <p style="margin: 0.15rem 0 0 0; color: var(--vw-text-secondary); font-size: 0.7rem;">{{ __('Unified Bible data - automatically synced from your settings') }}</p>
            </div>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <button type="button"
                        ...
                        style="padding: 0.3rem 0.6rem; background: rgba(6,182,212,0.2); border: 1px solid rgba(6,182,212,0.4); border-radius: 0.35rem; color: #0891b2; font-size: 0.65rem; cursor: pointer; font-weight: 500; display: flex; align-items: center; gap: 0.25rem;">
        ...
        <div style="padding: 0.5rem 1rem; background: rgba(6,182,212,0.08); border-bottom: 1px solid rgba(6,182,212,0.2); display: flex; gap: 1.5rem; flex-wrap: wrap;">
            <div style="display: flex; align-items: center; gap: 0.35rem;">
                <span style="color: var(--vw-text-secondary); font-size: 0.65rem;">{{ __('Scenes:') }}</span>
                <span style="color: #0891b2; font-weight: 600; font-size: 0.7rem;">{{ $summary['totalScenes'] ?? 0 }}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.35rem;">
                <span style="color: var(--vw-text-secondary); font-size: 0.65rem;">{{ __('With Characters:') }}</span>
                <span style="color: var(--vw-primary); font-weight: 600; font-size: 0.7rem;">{{ $summary['scenesWithCharacters'] ?? 0 }}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.35rem;">
                <span style="color: var(--vw-text-secondary); font-size: 0.65rem;">{{ __('With Locations:') }}</span>
                <span style="color: #34d399; font-weight: 600; font-size: 0.7rem;">{{ $summary['scenesWithLocations'] ?? 0 }}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.35rem;">
                <span style="color: var(--vw-text-secondary); font-size: 0.65rem;">{{ __('Unique Characters:') }}</span>
                <span style="color: #f472b6; font-weight: 600; font-size: 0.7rem;">{{ $summary['uniqueCharacters'] ?? 0 }}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.35rem;">
                <span style="color: var(--vw-text-secondary); font-size: 0.65rem;">{{ __('Continuity Issues:') }}</span>
            ...
            @if(!empty($sceneDNA['lastSyncedAt']))
                <div style="margin-left: auto; display: flex; align-items: center; gap: 0.35rem;">
                    <span style="color: var(--vw-text-secondary); font-size: 0.6rem;">{{ __('Last synced:') }}</span>
                    <span style="color: var(--vw-text-secondary); font-size: 0.6rem;">{{ \Carbon\Carbon::parse($sceneDNA['lastSyncedAt'])->diffForHumans() }}</span>
                </div>
            @endif
        </div>
        ...
        <div style="flex: 1; overflow-y: auto; padding: 0.75rem;">
            {{-- Tabs and Content --}}
                ...
                <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                    {{-- Tabs --}}
                    <div style="display: flex; gap: 0.25rem; border-bottom: 1px solid var(--vw-border); padding-bottom: 0.5rem; flex-wrap: wrap;">
                        <button type="button"
                                wire:click="$set('sceneDNAActiveTab', 'overview')"
                                style="padding: 0.4rem 0.8rem; background: {{ $activeTab === 'overview' ? 'rgba(6,182,212,0.2)' : 'transparent' }}; border: 1px solid {{ $activeTab === 'overview' ? 'rgba(6,182,212,0.4)' : 'transparent' }}; border-radius: 0.35rem; color: {{ $activeTab === 'overview' ? '#0891b2' : 'var(--vw-text-secondary)' }}; font-size: 0.7rem; cursor: pointer;">
                            {{ __('Scene Overview') }}
                        </button>
                        <button type="button"
                                wire:click="$set('sceneDNAActiveTab', 'style')"
                                style="padding: 0.4rem 0.8rem; background: {{ $activeTab === 'style' ? 'rgba(236,72,153,0.2)' : 'transparent' }}; border: 1px solid {{ $activeTab === 'style' ? 'rgba(236,72,153,0.4)' : 'transparent' }}; border-radius: 0.35rem; color: {{ $activeTab === 'style' ? '#be185d' : 'var(--vw-text-secondary)' }}; font-size: 0.7rem; cursor: pointer; display: flex; align-items: center; gap: 0.3rem;">
                            &#x1F3A8; {{ __('Global Style') }}
                        ...
                        <button type="button"
                                ...
                                style="padding: 0.4rem 0.8rem; background: {{ $activeTab === 'affinities' ? 'rgba(6,182,212,0.2)' : 'transparent' }}; border: 1px solid {{ $activeTab === 'affinities' ? 'rgba(6,182,212,0.4)' : 'transparent' }}; border-radius: 0.35rem; color: {{ $activeTab === 'affinities' ? '#0891b2' : 'var(--vw-text-secondary)' }}; font-size: 0.7rem; cursor: pointer;">
                            {{ __('Affinities') }}
                        </button>
                    </div>
                    ...
                    @if($activeTab === 'style')
                        ...
                        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                            ...
                            {{-- Quick Templates --}}
                            <div>
                                <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.35rem;">{{ __('Quick Templates') }}</label>
                                <div style="display: flex; gap: 0.3rem; flex-wrap: wrap;">
                                    <button type="button" wire:click="applyStyleTemplate('cinematic')" style="padding: 0.3rem 0.6rem; background: rgba(var(--vw-primary-rgb), 0.06); border: 1px solid rgba(var(--vw-primary-rgb), 0.12); border-radius: 0.3rem; color: var(--vw-text-secondary); font-size: 0.65rem; cursor: pointer;">&#x1F3AC; {{ __('Cinematic') }}</button>
                                    <button type="button" wire:click="applyStyleTemplate('documentary')" style="padding: 0.3rem 0.6rem; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); border-radius: 0.3rem; color: #16a34a; font-size: 0.65rem; cursor: pointer;">&#x1F3A5; {{ __('Documentary') }}</button>
                                    <button type="button" wire:click="applyStyleTemplate('anime')" style="padding: 0.3rem 0.6rem; background: rgba(236,72,153,0.15); border: 1px solid rgba(236,72,153,0.3); border-radius: 0.3rem; color: #be185d; font-size: 0.65rem; cursor: pointer;">&#x1F38C; {{ __('Anime') }}</button>
                                    <button type="button" wire:click="applyStyleTemplate('noir')" style="padding: 0.3rem 0.6rem; background: rgba(0,0,0,0.04); border: 1px solid var(--vw-border); border-radius: 0.3rem; color: var(--vw-text); font-size: 0.65rem; cursor: pointer;">&#x1F5A4; {{ __('Film Noir') }}</button>
                                    <button type="button" wire:click="applyStyleTemplate('photorealistic')" style="padding: 0.3rem 0.6rem; background: rgba(6,182,212,0.15); border: 1px solid rgba(6,182,212,0.3); border-radius: 0.3rem; color: #0891b2; font-size: 0.65rem; cursor: pointer;">&#x1F4F7; {{ __('Photorealistic') }}</button>
                                </div>
                            </div>
                            ...
                            <div style="padding: 0.5rem; background: rgba(0,0,0,0.02); border: 1px solid rgba(0,0,0,0.04); border-radius: 0.4rem;">
                                <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.4rem;">&#x1F4A1; {{ __('Lighting') }}</label>
                    ...
                    @elseif($activeTab === 'overview')
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 0.5rem;">
                            @foreach(($sceneDNA['scenes'] ?? []) as $sceneIndex => $sceneData)
                                <div style="background: rgba(0,0,0,0.02); border: 1px solid var(--vw-border); border-radius: 0.5rem; padding: 0.6rem; display: flex; flex-direction: column; gap: 0.4rem;">
                                    {{-- Scene Header --}}
                                    <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                        ...
                                        <div style="display: flex; gap: 0.25rem;">
                                            @if(!empty($sceneData['characters']))
                                                <span style="background: rgba(var(--vw-primary-rgb), 0.2); color: var(--vw-text-secondary); padding: 0.15rem 0.35rem; border-radius: 0.25rem; font-size: 0.55rem;">
                                                    {{ count($sceneData['characters']) }} {{ __('char') }}
                                                </span>
                                            @endif
                                            @if(!empty($sceneData['location']))
                                                <span style="background: rgba(52,211,153,0.2); color: #16a34a; padding: 0.15rem 0.35rem; border-radius: 0.25rem; font-size: 0.55rem;">
                                                    {{ __('loc') }}
                                                </span>
                                            @endif
                                        </div>
                                    ...
                                    @if(!empty($sceneData['characterNames']))
                                        <div style="display: flex; flex-wrap: wrap; gap: 0.2rem;">
                                            @foreach($sceneData['characterNames'] as $charName)
                                                <span style="background: rgba(var(--vw-primary-rgb), 0.06); border: 1px solid rgba(var(--vw-primary-rgb), 0.12); color: var(--vw-primary); padding: 0.1rem 0.3rem; border-radius: 0.25rem; font-size: 0.55rem;">
                                                    {{ $charName }}
                                                </span>
                                            @endforeach
                                        </div>
                                    ...
                                    {{-- Location --}}
                                    @if(!empty($sceneData['location']))
                                        <div style="display: flex; align-items: center; gap: 0.35rem; padding-top: 0.2rem; border-top: 1px solid rgba(0,0,0,0.03);">
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">&#x1F4CD;</span>
                                            <span style="color: var(--vw-text); font-size: 0.6rem;">{{ $sceneData['location']['name'] ?? __('Unknown') }}</span>
                                            @if(!empty($sceneData['location']['timeOfDay']))
                                                <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">{{ $sceneData['location']['timeOfDay'] }}</span>
                                            @endif
                                        </div>
                                    @endif
                                    ...
                                    @if(!empty($sceneData['style']))
                                        <div style="display: flex; align-items: center; gap: 0.25rem;">
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">&#x1F3A8;</span>
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">{{ __('Style applied') }}</span>
                                        </div>
                                    @endif
                                </div>
                            @endforeach
                        ...
                        @if(empty($sceneDNA['scenes']))
                            <div style="padding: 2rem; text-align: center; color: var(--vw-text-secondary); font-size: 0.75rem;">
                    ...
                    @elseif($activeTab === 'continuity')
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                @foreach($continuityIssues as $issue)
                                    <div style="background: rgba(249,115,22,0.1); border: 1px solid rgba(249,115,22,0.3); border-radius: 0.5rem; padding: 0.6rem;">
                                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.35rem;">
                                            <div style="display: flex; align-items: center; gap: 0.35rem;">
                                                ...
                                                @else
                                                    <span style="font-size: 0.8rem;">&#x26A0;</span>
                                                @endif
                                                <span style="color: #c2410c; font-weight: 600; font-size: 0.7rem;">
                                        ...
                                        <p style="color: var(--vw-text); font-size: 0.65rem; margin: 0;">
                                            {{ $issue['message'] ?? '' }}
                                        </p>
                        ...
                        @else
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem 1rem; text-align: center;">
                                ...
                                <p style="color: var(--vw-text-secondary); font-size: 0.7rem;">
                    ...
                    @elseif($activeTab === 'affinities')
                        ...
                        @if(!empty($affinities))
                            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                @foreach($affinities as $charName => $charData)
                                    ...
                                    @endphp
                                    <div style="background: rgba(0,0,0,0.02); border: 1px solid var(--vw-border); border-radius: 0.5rem; padding: 0.6rem;">
                                        ...
                                        <div style="display: flex; flex-wrap: wrap; gap: 0.35rem;">
                                            @foreach($locations as $locName => $count)
                                                <div style="background: rgba(52,211,153,0.15); border: 1px solid {{ $locName === $primaryLoc ? 'rgba(52,211,153,0.6)' : 'rgba(52,211,153,0.3)' }}; border-radius: 0.35rem; padding: 0.25rem 0.5rem; display: flex; align-items: center; gap: 0.3rem;">
                                                    <span style="color: #16a34a; font-size: 0.6rem;">{{ $locName }}</span>
                        ...
                        @else
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem 1rem; text-align: center;">
                                <div style="width: 60px; height: 60px; background: rgba(var(--vw-primary-rgb), 0.08); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 0.75rem;">
                                    <span style="font-size: 1.5rem;">&#x1F465;</span>
                                </div>
                                <h4 style="color: var(--vw-text); margin: 0 0 0.35rem 0; font-size: 0.9rem;">{{ __('No Affinities Yet') }}</h4>
                                <p style="color: var(--vw-text-secondary); font-size: 0.7rem;">
            ...
            <div style="padding: 0.5rem 1rem; border-top: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; background: rgba(0,0,0,0.2);">
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <label style="display: flex; align-items: center; gap: 0.35rem; cursor: pointer;">
                        ...
                        <span style="color: var(--vw-text); font-size: 0.65rem;">{{ __('Auto-sync when Bibles change') }}</span>
                    </label>
                </div>
