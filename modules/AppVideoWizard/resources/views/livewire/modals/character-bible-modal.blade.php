...
<div class="vw-modal-overlay"
    ...
    <div class="vw-modal"
        ...
        <div style="padding: 0.5rem 1rem; border-bottom: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            <div>
                <h3 style="margin: 0; color: var(--vw-text); font-size: 1rem; font-weight: 600;">{{ __('Character Bible') }}</h3>
                <p style="margin: 0.15rem 0 0 0; color: var(--vw-text-secondary); font-size: 0.7rem;">{{ __('Define consistent character appearances with reference images') }}</p>
            </div>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                @if(!empty($storyBibleCharacters) && $storyBibleStatus === 'ready')
                    <button type="button"
                        ...
                        <span style="background: var(--vw-border); padding: 0.1rem 0.3rem; border-radius: 0.25rem; font-size: 0.55rem;">{{ count($storyBibleCharacters) }}</span>
                    </button>
                @endif
                <button type="button" wire:click="closeModal" style="background: none; border: none; color: var(--vw-text-secondary); font-size: 1.25rem; cursor: pointer; padding: 0.25rem; line-height: 1;">&times;</button>
            </div>
        </div>

        {{-- Story Bible sync indicator with loading state --}}
        @if($isSyncingCharacterBible ?? false)
            <div style="padding: 0.5rem 1rem; background: rgba(var(--vw-primary-rgb), 0.06); border-bottom: 1px solid rgba(var(--vw-primary-rgb), 0.12); color: var(--vw-text-secondary); font-size: 0.7rem; display: flex; align-items: center; gap: 0.5rem;">
                <div style="width: 14px; height: 14px; border: 2px solid rgba(var(--vw-primary-rgb), 0.12); border-top-color: var(--vw-primary); border-radius: 50%; animation: vw-spin 0.8s linear infinite;"></div>
        ...
        <div style="flex: 1; overflow-y: auto; padding: 0.75rem; display: flex; gap: 0.75rem;">
            ...
            <div style="width: 190px; flex-shrink: 0; border-right: 1px solid var(--vw-border); padding-right: 0.75rem;">
                <button type="button"
                        wire:click="addCharacter"
                        style="width: 100%; padding: 0.4rem; background: transparent; border: 2px dashed var(--vw-border-accent); border-radius: 0.375rem; color: var(--vw-text-secondary); font-size: 0.7rem; cursor: pointer; margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: center; gap: 0.25rem;">
                ...
                @if(count($scriptScenes ?? []) > 0)
                    <button type="button"
                            ...
                            style="width: 100%; padding: 0.35rem; background: rgba(0,0,0,0.03); border: 1px solid var(--vw-border); border-radius: 0.375rem; color: var(--vw-text); font-size: 0.6rem; cursor: pointer; margin-bottom: 0.5rem;">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.35rem; max-height: 380px; overflow-y: auto;">
                    @forelse($characterBible['characters'] ?? [] as $index => $character)
                        <div wire:click="editCharacter({{ $index }})"
                            ...
                            <div style="width: 35px; height: 45px; border-radius: 0.25rem; overflow: hidden; background: rgba(0,0,0,0.05); flex-shrink: 0; display: flex; align-items: center; justify-content: center;">
                                ...
                                @else
                                    <span style="color: var(--vw-text-secondary); font-size: 0.9rem;">?</span>
                                @endif
                            </div>
                    ...
                    @empty
                        <div style="padding: 1rem; color: var(--vw-text-secondary); font-size: 0.65rem; text-align: center;">
                ...
                @if($currentChar)
                    <div style="display: flex; gap: 0.75rem;">
                        ...
                        <div style="width: 130px; flex-shrink: 0;">
                            <div style="width: 130px; height: 155px; border-radius: 0.5rem; overflow: hidden; background: rgba(0,0,0,0.06); border: 1px solid var(--vw-border); display: flex; align-items: center; justify-content: center; position: relative;">
                                ...
                                @elseif($isGeneratingPortrait ?? false)
                                    <div style="display: flex; flex-direction: column; align-items: center; gap: 0.35rem;">
                                        <div style="width: 20px; height: 20px; border: 2px solid rgba(var(--vw-primary-rgb), 0.12); border-top-color: var(--vw-primary); border-radius: 50%; animation: vw-spin 0.8s linear infinite;"></div>
                                        <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">{{ __('Generating...') }}</span>
                                    </div>
                                @else
                                    <div style="display: flex; flex-direction: column; align-items: center; gap: 0.35rem; color: var(--vw-text-secondary);">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.25rem; margin-top: 0.35rem;">
                                ...
                                <button type="button"
                                        ...
                                        style="width: 100%; padding: 0.35rem; background: linear-gradient(135deg, var(--vw-primary), #06b6d4); border: none; border-radius: 0.3rem; color: white; font-size: 0.6rem; cursor: pointer; font-weight: 600;">
                                ...
                                <div x-data="{ uploading: false }" style="position: relative;">
                                    ...
                                    <button type="button"
                                            style="width: 100%; padding: 0.35rem; background: var(--vw-border); border: 1px solid var(--vw-border); border-radius: 0.3rem; color: var(--vw-text); font-size: 0.6rem; cursor: pointer; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 0.25rem;">
                                ...
                                @if(!empty($currentChar['referenceImage']) && !empty($currentChar['referenceImageSource']))
                                    <div style="text-align: center; font-size: 0.5rem; color: var(--vw-text-secondary);">
                        ...
                        <div style="flex: 1;">
                            ...
                            <div style="margin-bottom: 0.4rem;">
                                <label style="display: block; color: var(--vw-text-secondary); font-size: 0.6rem; margin-bottom: 0.15rem;">{{ __('Character Name') }}</label>
                                <input type="text"
                            ...
                            {{-- Quick Templates --}}
                            <div style="margin-bottom: 0.4rem;">
                                <label style="display: block; color: var(--vw-text-secondary); font-size: 0.55rem; margin-bottom: 0.2rem;">{{ __('Quick Templates') }}</label>
                                <div style="display: flex; gap: 0.25rem; flex-wrap: wrap;">
                                    <button type="button" wire:click="applyCharacterTemplate({{ $editIndex }}, 'action-hero')" style="padding: 0.2rem 0.4rem; background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.3); border-radius: 0.2rem; color: #dc2626; font-size: 0.55rem; cursor: pointer;">{{ __('Action Hero') }}</button>
                                    <button type="button" wire:click="applyCharacterTemplate({{ $editIndex }}, 'tech-pro')" style="padding: 0.2rem 0.4rem; background: rgba(6,182,212,0.15); border: 1px solid rgba(6,182,212,0.3); border-radius: 0.2rem; color: #0891b2; font-size: 0.55rem; cursor: pointer;">{{ __('Tech Pro') }}</button>
                                    <button type="button" wire:click="applyCharacterTemplate({{ $editIndex }}, 'mysterious')" style="padding: 0.2rem 0.4rem; background: rgba(var(--vw-primary-rgb), 0.06); border: 1px solid rgba(var(--vw-primary-rgb), 0.12); border-radius: 0.2rem; color: var(--vw-text-secondary); font-size: 0.55rem; cursor: pointer;">{{ __('Mysterious') }}</button>
                            ...
                            <div x-data="{ traitsOpen: false, newTrait: '' }" style="margin-bottom: 0.4rem;">
                                <button type="button"
                                    ...
                                    <span style="color: var(--vw-text-secondary); font-size: 0.6rem; transition: transform 0.2s;" :style="traitsOpen ? '' : 'transform: rotate(-90deg)'">V</span>
                                    <span style="color: var(--vw-text-secondary); font-size: 0.6rem;">{{ __('Character Traits') }}</span>
                                    <span style="color: var(--vw-text-secondary); font-size: 0.5rem; margin-left: 0.2rem;">({{ count($currentChar['traits'] ?? []) }})</span>
                                </button>
                                ...
                                <div x-show="traitsOpen" x-collapse>
                                    ...
                                    <div style="display: flex; flex-wrap: wrap; gap: 0.25rem; margin-bottom: 0.35rem;">
                                        @forelse($currentChar['traits'] ?? [] as $traitIdx => $trait)
                                            <span style="display: inline-flex; align-items: center; gap: 0.2rem; padding: 0.2rem 0.4rem; background: rgba(var(--vw-primary-rgb), 0.08); border: 1px solid var(--vw-border-accent); border-radius: 0.75rem; color: var(--vw-text-secondary); font-size: 0.55rem;">
                                                {{ $trait }}
                                                <button type="button"
                                                        wire:click="removeCharacterTrait({{ $editIndex }}, {{ $traitIdx }})"
                                                        style="background: none; border: none; color: var(--vw-text-secondary); cursor: pointer; padding: 0; line-height: 1; font-size: 0.6rem;"
                                                        title="{{ __('Remove') }}">&times;</button>
                                            </span>
                                        @empty
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem; font-style: italic;">{{ __('No traits added yet') }}</span>
                                        @endforelse
                                    </div>
                                    ...
                                    <div style="display: flex; flex-wrap: wrap; gap: 0.2rem;">
                                        <span style="color: var(--vw-text-secondary); font-size: 0.5rem; margin-right: 0.2rem;">{{ __('Presets:') }}</span>
                                        <button type="button" wire:click="applyTraitPreset({{ $editIndex }}, 'hero')" style="padding: 0.15rem 0.3rem; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2); border-radius: 0.2rem; color: #dc2626; font-size: 0.5rem; cursor: pointer;">{{ __('Hero') }}</button>
                                        <button type="button" wire:click="applyTraitPreset({{ $editIndex }}, 'villain')" style="padding: 0.15rem 0.3rem; background: rgba(var(--vw-primary-rgb), 0.04); border: 1px solid rgba(var(--vw-primary-rgb), 0.08); border-radius: 0.2rem; color: var(--vw-text-secondary); font-size: 0.5rem; cursor: pointer;">{{ __('Villain') }}</button>
                                        ...
                                        <button type="button" wire:click="applyTraitPreset({{ $editIndex }}, 'professional')" style="padding: 0.15rem 0.3rem; background: rgba(6,182,212,0.1); border: 1px solid rgba(6,182,212,0.2); border-radius: 0.2rem; color: #0891b2; font-size: 0.5rem; cursor: pointer;">{{ __('Pro') }}</button>
                                        <button type="button" wire:click="applyTraitPreset({{ $editIndex }}, 'mysterious')" style="padding: 0.15rem 0.3rem; background: rgba(3,252,244,0.1); border: 1px solid rgba(3,252,244,0.2); border-radius: 0.2rem; color: #67e8f9; font-size: 0.5rem; cursor: pointer;">{{ __('Mysterious') }}</button>
                                        <button type="button" wire:click="applyTraitPreset({{ $editIndex }}, 'comic')" style="padding: 0.15rem 0.3rem; background: rgba(16,185,129,0.1); border: 1px solid rgba(16,185,129,0.2); border-radius: 0.2rem; color: #16a34a; font-size: 0.5rem; cursor: pointer;">{{ __('Comic') }}</button>
                            ...
                            <div x-data="{ dnaOpen: false, presetsOpen: false }" style="margin-bottom: 0.4rem; padding: 0.35rem; background: rgba(var(--vw-primary-rgb), 0.04); border: 1px solid rgba(var(--vw-primary-rgb), 0.08); border-radius: 0.35rem;">
                                {{-- DNA Section Header --}}
                                <button type="button"
                                    ...
                                    <span style="color: var(--vw-text-secondary); font-size: 0.6rem; transition: transform 0.2s;" :style="dnaOpen ? '' : 'transform: rotate(-90deg)'">V</span>
                                    <span style="color: var(--vw-text-secondary); font-size: 0.65rem; font-weight: 600;">{{ __('Character DNA') }}</span>
                                    <span style="color: var(--vw-text-secondary); font-size: 0.5rem; margin-left: 0.2rem;">({{ __('Hair, Wardrobe, Makeup, Accessories') }})</span>
                                    @if($hasDNA)
                                ...
                                <div x-show="dnaOpen" x-collapse style="margin-top: 0.35rem;">
                                    ...
                                    <div style="margin-bottom: 0.4rem;">
                                        <button type="button"
                                                ...
                                                style="width: 100%; padding: 0.4rem 0.5rem; background: linear-gradient(135deg, rgba(16,185,129,0.25), rgba(6,182,212,0.2)); border: 1px solid rgba(16,185,129,0.5); border-radius: 0.3rem; color: #16a34a; font-size: 0.6rem; font-weight: 600; cursor: {{ $hasPortrait && $portraitReady ? 'pointer' : 'not-allowed' }}; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 0.3rem; {{ !$hasPortrait || !$portraitReady ? 'opacity: 0.5;' : '' }}"
                                        ...
                                        @if(!$hasPortrait || !$portraitReady)
                                            <p style="color: var(--vw-text-secondary); font-size: 0.45rem; text-align: center; margin-top: 0.2rem;">{{ __('Generate a portrait first to extract DNA') }}</p>
                                        @else
                                    ...
                                    <div style="margin-bottom: 0.35rem;">
                                        <button type="button"
                                            ...
                                            <span style="color: var(--vw-text-secondary); font-size: 0.5rem; transition: transform 0.2s;" :style="presetsOpen ? '' : 'transform: rotate(-90deg)'">V</span>
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">{{ __('Quick Presets') }}</span>
                                        </button>
                                        <div x-show="presetsOpen" x-collapse style="display: flex; flex-wrap: wrap; gap: 0.2rem;">
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'corporate-female')" style="padding: 0.15rem 0.3rem; background: rgba(3,252,244,0.15); border: 1px solid rgba(3,252,244,0.3); border-radius: 0.2rem; color: #67e8f9; font-size: 0.45rem; cursor: pointer;">{{ __('Corp F') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'corporate-male')" style="padding: 0.15rem 0.3rem; background: rgba(3,252,244,0.15); border: 1px solid rgba(3,252,244,0.3); border-radius: 0.2rem; color: #67e8f9; font-size: 0.45rem; cursor: pointer;">{{ __('Corp M') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'tech-female')" style="padding: 0.15rem 0.3rem; background: rgba(6,182,212,0.15); border: 1px solid rgba(6,182,212,0.3); border-radius: 0.2rem; color: #0891b2; font-size: 0.45rem; cursor: pointer;">{{ __('Tech F') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'tech-male')" style="padding: 0.15rem 0.3rem; background: rgba(6,182,212,0.15); border: 1px solid rgba(6,182,212,0.3); border-radius: 0.2rem; color: #0891b2; font-size: 0.45rem; cursor: pointer;">{{ __('Tech M') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'action-hero-female')" style="padding: 0.15rem 0.3rem; background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.3); border-radius: 0.2rem; color: #dc2626; font-size: 0.45rem; cursor: pointer;">{{ __('Action F') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'action-hero-male')" style="padding: 0.15rem 0.3rem; background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.3); border-radius: 0.2rem; color: #dc2626; font-size: 0.45rem; cursor: pointer;">{{ __('Action M') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'scientist-female')" style="padding: 0.15rem 0.3rem; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); border-radius: 0.2rem; color: #16a34a; font-size: 0.45rem; cursor: pointer;">{{ __('Sci F') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'scientist-male')" style="padding: 0.15rem 0.3rem; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); border-radius: 0.2rem; color: #16a34a; font-size: 0.45rem; cursor: pointer;">{{ __('Sci M') }}</button>
                                            <button type="button" wire:click="applyCharacterLookPreset({{ $editIndex }}, 'cyberpunk')" style="padding: 0.15rem 0.3rem; background: rgba(236,72,153,0.15); border: 1px solid rgba(236,72,153,0.3); border-radius: 0.2rem; color: #be185d; font-size: 0.45rem; cursor: pointer;">{{ __('Cyber') }}</button>
                                    ...
                                    {{-- Hair (inline compact) --}}
                                    <div style="margin-bottom: 0.3rem; padding: 0.25rem; background: rgba(0,0,0,0.02); border-radius: 0.25rem;">
                                        <div style="display: flex; align-items: center; gap: 0.3rem; margin-bottom: 0.2rem;">
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem; min-width: 2rem;">{{ __('Hair') }}</span>
                                    ...
                                    <div style="margin-bottom: 0.3rem; padding: 0.25rem; background: rgba(0,0,0,0.02); border-radius: 0.25rem;">
                                        <div style="display: flex; align-items: center; gap: 0.3rem; margin-bottom: 0.2rem;">
                                            <span style="color: var(--vw-text-secondary); font-size: 0.55rem; min-width: 2rem;">{{ __('Wardrobe') }}</span>
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.25rem;">
                                        {{-- Makeup --}}
                                        <div style="padding: 0.25rem; background: rgba(0,0,0,0.02); border-radius: 0.25rem;">
                                            <div style="display: flex; align-items: center; gap: 0.2rem; margin-bottom: 0.2rem;">
                                                <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">{{ __('Makeup') }}</span>
                                        ...
                                        {{-- Accessories --}}
                                        <div x-data="{ newAcc: '' }" style="padding: 0.25rem; background: rgba(0,0,0,0.02); border-radius: 0.25rem;">
                                            <div style="display: flex; align-items: center; gap: 0.2rem; margin-bottom: 0.2rem;">
                                                <span style="color: var(--vw-text-secondary); font-size: 0.55rem;">{{ __('Accessories') }}</span>
                                                <span style="color: var(--vw-text-secondary); font-size: 0.45rem;">({{ count($currentChar['accessories'] ?? []) }})</span>
                                            </div>
                                            <div style="display: flex; flex-wrap: wrap; gap: 0.15rem; margin-bottom: 0.15rem; min-height: 18px;">
                                                @foreach($currentChar['accessories'] ?? [] as $accIdx => $accessory)
                                                    <span style="display: inline-flex; align-items: center; gap: 0.1rem; padding: 0.1rem 0.25rem; background: rgba(251,191,36,0.2); border: 1px solid rgba(251,191,36,0.4); border-radius: 0.5rem; color: #fcd34d; font-size: 0.45rem;">
                                                        {{ $accessory }}
                                                        <button type="button" wire:click="removeCharacterAccessory({{ $editIndex }}, {{ $accIdx }})" style="background: none; border: none; color: var(--vw-text-secondary); cursor: pointer; padding: 0; line-height: 1; font-size: 0.5rem;">&times;</button>
                                                    </span>
                                                @endforeach
                                            </div>
                            ...
                            <div x-data="{ voiceOpen: false }" style="margin-bottom: 0.4rem; padding: 0.35rem; background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.2); border-radius: 0.35rem;">
                                ...
                                <button type="button"
                                        ...
                                        style="display: flex; align-items: center; gap: 0.25rem; width: 100%; background: none; border: none; padding: 0; cursor: pointer;">
                                    <span style="color: #16a34a; font-size: 0.6rem; transition: transform 0.2s;" :style="voiceOpen ? '' : 'transform: rotate(-90deg)'">V</span>
                                    <span style="color: #16a34a; font-size: 0.65rem; font-weight: 600;">{{ __('Character Voice') }}</span>
                                    <span style="color: var(--vw-text-secondary); font-size: 0.5rem; margin-left: 0.2rem;">({{ __('TTS & Lip-sync') }})</span>
                                    @if($hasVoice)
                                ...
                                <div x-show="voiceOpen" x-collapse style="margin-top: 0.35rem;">
                                    {{-- Voice Presets --}}
                                    <div style="margin-bottom: 0.35rem;">
                                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.5rem; margin-bottom: 0.15rem;">{{ __('Voice Presets') }}</label>
                                        <div style="display: flex; flex-wrap: wrap; gap: 0.2rem;">
                                            <button type="button" wire:click="applyCharacterVoicePreset({{ $editIndex }}, 'hero-male')" style="padding: 0.15rem 0.3rem; background: rgba(6,182,212,0.15); border: 1px solid rgba(6,182,212,0.3); border-radius: 0.2rem; color: #0891b2; font-size: 0.45rem; cursor: pointer;">{{ __('Hero M') }}</button>
                                            <button type="button" wire:click="applyCharacterVoicePreset({{ $editIndex }}, 'hero-female')" style="padding: 0.15rem 0.3rem; background: rgba(236,72,153,0.15); border: 1px solid rgba(236,72,153,0.3); border-radius: 0.2rem; color: #be185d; font-size: 0.45rem; cursor: pointer;">{{ __('Hero F') }}</button>
                                            <button type="button" wire:click="applyCharacterVoicePreset({{ $editIndex }}, 'villain-male')" style="padding: 0.15rem 0.3rem; background: rgba(var(--vw-primary-rgb), 0.06); border: 1px solid rgba(var(--vw-primary-rgb), 0.12); border-radius: 0.2rem; color: var(--vw-text-secondary); font-size: 0.45rem; cursor: pointer;">{{ __('Villain M') }}</button>
                                            <button type="button" wire:click="applyCharacterVoicePreset({{ $editIndex }}, 'villain-female')" style="padding: 0.15rem 0.3rem; background: rgba(var(--vw-primary-rgb), 0.06); border: 1px solid rgba(var(--vw-primary-rgb), 0.12); border-radius: 0.2rem; color: var(--vw-text-secondary); font-size: 0.45rem; cursor: pointer;">{{ __('Villain F') }}</button>
                                            ...
                                            <button type="button" wire:click="applyCharacterVoicePreset({{ $editIndex }}, 'narrator')" style="padding: 0.15rem 0.3rem; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); border-radius: 0.2rem; color: #16a34a; font-size: 0.45rem; cursor: pointer;">{{ __('Narrator') }}</button>
                                    ...
                                    <div style="margin-bottom: 0.35rem;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.1rem;">
                                            <label style="color: var(--vw-text-secondary); font-size: 0.5rem;">{{ __('Speed') }}</label>
                                            <span style="color: var(--vw-text-secondary); font-size: 0.5rem;">{{ number_format($voiceSettings['speed'] ?? 1.0, 2) }}x</span>
                                        </div>
                                        <input type="range"
                                    ...
                                    {{-- Emotion Preview --}}
                                    <div style="margin-bottom: 0.35rem; padding-top: 0.3rem; border-top: 1px solid var(--vw-border);">
                                        ...
                                        <div style="display: flex; gap: 0.3rem; align-items: center;">
                                            ...
                                            <button type="button"
                                                    ...
                                                    style="padding: 0.25rem 0.5rem; background: linear-gradient(135deg, rgba(var(--vw-primary-rgb), 0.12), rgba(16,185,129,0.3)); border: 1px solid var(--vw-border-accent); border-radius: 0.25rem; color: var(--vw-text-secondary); font-size: 0.55rem; cursor: pointer; font-weight: 500; display: flex; align-items: center; gap: 0.2rem;">
                                        ...
                                        <p style="color: var(--vw-text-secondary); font-size: 0.4rem; margin-top: 0.15rem;">
                                            {{ __('Hear how this voice sounds with emotional direction') }}
                                        </p>
                                    </div>

                                    {{-- Speaking Role & Narrator (side by side) --}}
                                    <div style="display: grid; grid-template-columns: 1fr auto; gap: 0.5rem; padding-top: 0.3rem; border-top: 1px solid var(--vw-border);">
                                        {{-- Speaking Role --}}
                                        <div>
                                        ...
                                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: flex-end;">
                                            <label style="display: flex; align-items: center; gap: 0.3rem; cursor: pointer; padding: 0.25rem 0.4rem; background: {{ ($currentChar['isNarrator'] ?? false) ? 'rgba(251,191,36,0.25)' : 'rgba(0,0,0,0.03)' }}; border: 1px solid {{ ($currentChar['isNarrator'] ?? false) ? 'rgba(251,191,36,0.5)' : 'var(--vw-border)' }}; border-radius: 0.25rem;">
                                                <input type="checkbox"
                                                       wire:model.live="characterBible.characters.{{ $editIndex }}.isNarrator"
                                                       style="accent-color: #f59e0b; width: 12px; height: 12px;">
                                                <span style="color: {{ ($currentChar['isNarrator'] ?? false) ? '#fcd34d' : 'var(--vw-text-secondary)' }}; font-size: 0.55rem; font-weight: 500;">{{ __('Narrator') }}</span>
                                            </label>
                                        </div>
                                    </div>
                    ...
                    <div style="margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid var(--vw-border);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.3rem;">
                            <label style="color: var(--vw-text-secondary); font-size: 0.6rem;">{{ __('Appears in Scenes') }}</label>
                            @php
                            ...
                            @endphp
                            <span style="font-size: 0.55rem; color: {{ $appliedCount > 0 ? '#10b981' : 'var(--vw-text-secondary)' }};">
                        ...
                        @if($appliedCount === 0 && $totalScenes > 0)
                            <div style="padding: 0.35rem 0.5rem; margin-bottom: 0.3rem; background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 0.25rem;">
                                <span style="color: #dc2626; font-size: 0.55rem;">{{ __('Please assign at least one scene to this character') }}</span>
                            </div>
                        @endif
                        <div style="display: flex; flex-wrap: wrap; gap: 0.25rem;">
                            @foreach($scriptScenes ?? [] as $sceneIndex => $scene)
                                ...
                                <button type="button"
                                        ...
                                        style="width: 28px; height: 28px; border-radius: 0.3rem; border: 2px solid {{ $isApplied ? 'var(--vw-primary)' : 'var(--vw-border)' }}; background: {{ $isApplied ? 'var(--vw-primary)' : 'rgba(0,0,0,0.03)' }}; color: {{ $isApplied ? 'white' : 'var(--vw-text-secondary)' }}; cursor: pointer; font-size: 0.7rem; font-weight: {{ $isApplied ? '700' : '500' }}; transition: all 0.15s ease; {{ $isApplied ? 'box-shadow: 0 2px 8px var(--vw-border-accent);' : '' }}">
                                    {{ $sceneIndex + 1 }}
                                </button>
                            @endforeach
                            @if(count($scriptScenes ?? []) > 0)
        ...
        <div style="padding: 0.5rem 1rem; border-top: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            <label style="display: flex; align-items: center; gap: 0.4rem; color: var(--vw-text); font-size: 0.7rem; cursor: pointer;">
                <input type="checkbox" wire:model.live="characterBible.enabled" style="accent-color: var(--vw-primary);">
                {{ __('Enable Character Bible') }}
            </label>
            <button type="button"
                    wire:click="closeModal"
                    style="padding: 0.4rem 0.9rem; background: linear-gradient(135deg, var(--vw-primary), #06b6d4); border: none; border-radius: 0.35rem; color: white; font-weight: 600; cursor: pointer; font-size: 0.75rem;">
                {{ __('Save & Close') }}
            </button>
        </div>
    </div>
