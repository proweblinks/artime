...
<div class="vw-modal-overlay"
     ...
     style="position: fixed; inset: 0; background: rgba(0,0,0,0.4); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 0.5rem;">
    <div class="vw-modal"
        ...
        <div style="padding: 0.75rem 1rem; border-bottom: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; background: linear-gradient(135deg, rgba(251,191,36,0.1), rgba(245,158,11,0.05));">
            <div>
                <h3 style="margin: 0; color: var(--vw-text); font-size: 1.1rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem;">
        ...
        <div style="display: flex; gap: 0.25rem; padding: 0.5rem 1rem; border-bottom: 1px solid var(--vw-border); flex-shrink: 0; background: rgba(0,0,0,0.2); overflow-x: auto;">
            @foreach(['overview' => ['icon' => '📋', 'label' => 'Overview'], 'characters' => ['icon' => '👥', 'label' => 'Characters'], 'locations' => ['icon' => '🏛️', 'label' => 'Locations'], 'style' => ['icon' => '🎨', 'label' => 'Visual Style'], 'cinematography' => ['icon' => '🎬', 'label' => 'Cinematography']] as $tabKey => $tabInfo)
                <button type="button"
                        ...
                        style="padding: 0.4rem 0.75rem; border-radius: 0.375rem; font-size: 0.75rem; cursor: pointer; display: flex; align-items: center; gap: 0.3rem; transition: all 0.2s; border: 1px solid {{ $storyBibleTab === $tabKey ? 'rgba(251,191,36,0.5)' : 'transparent' }}; background: {{ $storyBibleTab === $tabKey ? 'rgba(251,191,36,0.15)' : 'transparent' }}; color: {{ $storyBibleTab === $tabKey ? '#fcd34d' : 'var(--vw-text-secondary)' }};">
                    <span>{{ $tabInfo['icon'] }}</span>
                    <span>{{ __($tabInfo['label']) }}</span>
                    @if($tabKey === 'characters')
                        <span style="background: var(--vw-border); padding: 0.1rem 0.3rem; border-radius: 0.25rem; font-size: 0.6rem;">{{ count($storyBible['characters'] ?? []) }}</span>
                    @elseif($tabKey === 'locations')
                        <span style="background: var(--vw-border); padding: 0.1rem 0.3rem; border-radius: 0.25rem; font-size: 0.6rem;">{{ count($storyBible['locations'] ?? []) }}</span>
                    @endif
                </button>
            @endforeach
        </div>
        ...
        <div style="flex: 1; overflow-y: auto; padding: 1rem; position: relative;">
            ...
            @if($storyBibleTab === 'overview')
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                        {{-- Title --}}
                        <div>
                            <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.25rem; text-transform: uppercase;">{{ __('Title') }}</label>
                            <input type="text"
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                            <div>
                                <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.25rem; text-transform: uppercase;">{{ __('Theme') }}</label>
                                <input type="text"
                        ...
                        {{-- Structure Template --}}
                        <div>
                            <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.25rem; text-transform: uppercase;">{{ __('Story Structure') }}</label>
                            <div style="display: flex; gap: 0.5rem;">
                                @foreach(['three-act' => '3-Act', 'five-act' => '5-Act', 'heros-journey' => "Hero's Journey"] as $key => $label)
                                    <button type="button"
                                            ...
                                            style="flex: 1; padding: 0.4rem 0.5rem; border-radius: 0.375rem; font-size: 0.7rem; cursor: pointer; border: 1px solid {{ ($storyBible['structureTemplate'] ?? 'three-act') === $key ? 'rgba(251,191,36,0.5)' : 'var(--vw-border)' }}; background: {{ ($storyBible['structureTemplate'] ?? 'three-act') === $key ? 'rgba(251,191,36,0.15)' : 'rgba(0,0,0,0.03)' }}; color: {{ ($storyBible['structureTemplate'] ?? 'three-act') === $key ? '#fcd34d' : 'var(--vw-text)' }};">
                                        {{ $label }}
                                    </button>
                                @endforeach
                            </div>
                    ...
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.5rem; text-transform: uppercase;">{{ __('Act Structure') }}</label>
                        <div style="display: flex; flex-direction: column; gap: 0.5rem; max-height: 280px; overflow-y: auto; padding-right: 0.25rem;">
                            @forelse($storyBible['acts'] ?? [] as $actIndex => $act)
                                <div style="padding: 0.5rem; background: rgba(0,0,0,0.03); border: 1px solid var(--vw-border); border-radius: 0.375rem;">
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.25rem;">
                                        ...
                                        <span style="color: var(--vw-text-secondary); font-size: 0.65rem;">{{ $act['percentage'] ?? 0 }}%</span>
                                    </div>
                                    <p style="margin: 0; color: var(--vw-text); font-size: 0.7rem; line-height: 1.4;">{{ $act['description'] ?? '' }}</p>
                            ...
                            @empty
                                <div style="padding: 1rem; text-align: center; color: var(--vw-text-secondary); font-size: 0.75rem;">
                ...
                {{-- Generate Button --}}
                <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--vw-border);">
                    <button type="button"
                        ...
                        @if($isGeneratingStoryBible)
                            <div style="width: 16px; height: 16px; border: 2px solid var(--vw-text-secondary); border-top-color: white; border-radius: 50%; animation: vw-spin 0.8s linear infinite;"></div>
                    ...
                    <p style="text-align: center; color: var(--vw-text-secondary); font-size: 0.65rem; margin-top: 0.5rem;">{{ __('AI will create title, characters, locations, and structure based on your concept') }}</p>
                </div>
            @endif
            ...
            @if($storyBibleTab === 'characters')
                <div style="display: flex; gap: 0.75rem; height: calc(100% - 40px);">
                    {{-- Characters List --}}
                    <div style="width: 200px; flex-shrink: 0; border-right: 1px solid var(--vw-border); padding-right: 0.75rem;">
                        <button type="button"
                    ...
                    <div style="flex: 1; overflow-y: auto;">
                        ...
                        @if($currentChar)
                            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                ...
                                <div>
                                    <label style="display: block; color: var(--vw-text-secondary); font-size: 0.6rem; margin-bottom: 0.15rem;">
                                ...
                                {{-- Traits --}}
                                <div x-data="{ newTrait: '' }">
                                    <label style="display: block; color: var(--vw-text-secondary); font-size: 0.6rem; margin-bottom: 0.15rem;">{{ __('Traits') }}</label>
                                    <div style="display: flex; flex-wrap: wrap; gap: 0.25rem; margin-bottom: 0.35rem;">
                                        @forelse($currentChar['traits'] ?? [] as $traitIdx => $trait)
                                            <span style="display: inline-flex; align-items: center; gap: 0.2rem; padding: 0.2rem 0.4rem; background: rgba(var(--vw-primary-rgb), 0.08); border: 1px solid var(--vw-border-accent); border-radius: 0.75rem; color: var(--vw-text-secondary); font-size: 0.6rem;">
                                                {{ $trait }}
                                                <button type="button" wire:click="removeBibleCharacterTrait({{ $editingBibleCharacterIndex }}, {{ $traitIdx }})" style="background: none; border: none; color: var(--vw-text-secondary); cursor: pointer; padding: 0; line-height: 1; font-size: 0.7rem;">&times;</button>
                                            </span>
                                        @empty
                                            <span style="color: var(--vw-text-secondary); font-size: 0.6rem; font-style: italic;">{{ __('No traits') }}</span>
                                        @endforelse
                                    </div>
                        ...
                        @else
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 200px; color: var(--vw-text-secondary);">
                ...
                <div style="display: flex; gap: 0.75rem; height: calc(100% - 40px);">
                    ...
                    <div style="width: 200px; flex-shrink: 0; border-right: 1px solid var(--vw-border); padding-right: 0.75rem;">
                        <button type="button"
                                ...
                                style="width: 100%; padding: 0.4rem; background: transparent; border: 2px dashed rgba(6,182,212,0.4); border-radius: 0.375rem; color: #0891b2; font-size: 0.7rem; cursor: pointer; margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: center; gap: 0.25rem;">
                    ...
                    <div style="flex: 1; overflow-y: auto;">
                        ...
                        @if($currentLoc)
                            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                {{-- Name --}}
                                <div>
                                    <label style="display: block; color: var(--vw-text-secondary); font-size: 0.6rem; margin-bottom: 0.15rem;">{{ __('Location Name') }}</label>
                                    <input type="text"
                                ...
                                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0.5rem;">
                                    ...
                                    <div>
                                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.6rem; margin-bottom: 0.15rem;">{{ __('Atmosphere') }}</label>
                                        <input type="text"
            ...
            @if($storyBibleTab === 'style')
                <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                    {{-- Visual Mode --}}
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.35rem; text-transform: uppercase;">{{ __('Visual Mode') }}</label>
                    ...
                    {{-- Pacing --}}
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.35rem; text-transform: uppercase;">{{ __('Pacing') }}</label>
                        <div style="display: flex; gap: 0.5rem;">
                            @foreach(['fast' => 'Fast', 'balanced' => 'Balanced', 'contemplative' => 'Contemplative'] as $pace => $label)
                                <button type="button"
                                        ...
                                        style="flex: 1; padding: 0.5rem; border-radius: 0.375rem; cursor: pointer; border: 1px solid {{ ($storyBible['pacing']['overall'] ?? 'balanced') === $pace ? 'rgba(16,185,129,0.5)' : 'var(--vw-border)' }}; background: {{ ($storyBible['pacing']['overall'] ?? 'balanced') === $pace ? 'rgba(16,185,129,0.15)' : 'rgba(0,0,0,0.02)' }}; color: {{ ($storyBible['pacing']['overall'] ?? 'balanced') === $pace ? '#16a34a' : 'var(--vw-text)' }}; font-size: 0.75rem;">
                                    {{ $label }}
                                </button>
                            @endforeach
                        </div>
                    ...
                    {{-- Emotional Beats --}}
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.25rem; text-transform: uppercase;">{{ __('Emotional Journey') }}</label>
                        <div style="display: flex; flex-wrap: wrap; gap: 0.25rem;">
                            @forelse($storyBible['pacing']['emotionalBeats'] ?? [] as $beat)
                                <span style="padding: 0.25rem 0.5rem; background: rgba(236,72,153,0.2); border: 1px solid rgba(236,72,153,0.4); border-radius: 0.375rem; color: #be185d; font-size: 0.7rem;">{{ $beat }}</span>
                            @empty
                                <span style="color: var(--vw-text-secondary); font-size: 0.7rem; font-style: italic;">{{ __('Generate Story Bible to define emotional journey') }}</span>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            @if($storyBibleTab === 'cinematography')
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    ...
                    <div style="padding: 0.75rem; background: rgba(236,72,153,0.1); border: 1px solid rgba(236,72,153,0.3); border-radius: 0.5rem;">
                        <p style="color: var(--vw-text); font-size: 0.75rem; margin: 0;">
                            <strong style="color: #be185d;">🎬 Cinematography Patterns</strong> control how scenes are decomposed into shots.
                    ...
                    {{-- Global Rules --}}
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.5rem; text-transform: uppercase;">{{ __('Global Cinematography Rules') }}</label>
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem;">
                            ...
                            <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem; background: rgba(0,0,0,0.02); border: 1px solid var(--vw-border); border-radius: 0.375rem;">
                    ...
                    {{-- Dialogue Settings --}}
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.5rem; text-transform: uppercase;">{{ __('Dialogue Pattern Settings') }}</label>
                    ...
                    {{-- Action Settings --}}
                    <div>
                        <label style="display: block; color: var(--vw-text-secondary); font-size: 0.65rem; margin-bottom: 0.5rem; text-transform: uppercase;">{{ __('Action Pattern Settings') }}</label>
                        <div style="padding: 0.75rem; background: rgba(0,0,0,0.02); border: 1px solid var(--vw-border); border-radius: 0.5rem;">
                            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem;">
                                <span style="color: var(--vw-text); font-size: 0.75rem;">{{ __('Default Pattern:') }}</span>
                    ...
                    <div>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; max-height: 200px; overflow-y: auto; padding: 0.5rem; background: rgba(0,0,0,0.2); border-radius: 0.5rem;">
                            ...
                            @foreach($patternCategories as $category => $patterns)
                                <div style="padding: 0.5rem; background: rgba(0,0,0,0.02); border-radius: 0.375rem;">
                                    <div style="font-weight: 600; color: #be185d; font-size: 0.7rem; margin-bottom: 0.35rem;">{{ $category }}</div>
                                    @foreach($patterns as $pattern)
                                        <div style="color: var(--vw-text-secondary); font-size: 0.65rem; padding: 0.15rem 0;">• {{ ucwords(str_replace('_', ' ', $pattern)) }}</div>
                                    @endforeach
                                </div>
                            @endforeach
                        </div>
        ...
        <div style="padding: 0.5rem 1rem; border-top: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; background: rgba(0,0,0,0.2);">
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                ...
                @else
                    <span style="color: var(--vw-text-secondary); font-size: 0.65rem; font-style: italic;">
