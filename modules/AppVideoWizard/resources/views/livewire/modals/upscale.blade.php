...
<div class="vw-modal-overlay"
    ...
    <div class="vw-modal"
         style="background: #ffffff; border: 1px solid rgba(251,191,36,0.3); border-radius: 1rem; width: 100%; max-width: 500px; overflow: hidden;">
        {{-- Header --}}
        <div style="padding: 1rem 1.25rem; border-bottom: 1px solid var(--vw-border); display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h3 style="margin: 0; color: var(--vw-text); font-size: 1.1rem; font-weight: 600;">⬆️ {{ __('Upscale Image') }}</h3>
                <p style="margin: 0.25rem 0 0 0; color: var(--vw-text-secondary); font-size: 0.8rem;">{{ __('Enhance image resolution and quality') }}</p>
            </div>
            <button type="button" wire:click="closeUpscaleModal" style="background: none; border: none; color: var(--vw-text-secondary); font-size: 1.5rem; cursor: pointer; padding: 0.25rem; line-height: 1;">&times;</button>
        </div>
        ...
        <div style="padding: 1.25rem;">
            ...
            @if($storyboardScene && !empty($storyboardScene['imageUrl']))
                <div style="margin-bottom: 1.25rem; text-align: center;">
                    <img src="{{ $storyboardScene['imageUrl'] }}"
                         ...
                         style="max-width: 100%; max-height: 200px; border-radius: 0.5rem; border: 1px solid var(--vw-border);">
            ...
            <div style="margin-bottom: 1.25rem;">
                <label style="display: block; color: var(--vw-text); font-size: 0.85rem; margin-bottom: 0.75rem;">{{ __('Select Output Quality') }}</label>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                    {{-- HD Option --}}
                    <button type="button"
                            wire:click="$set('upscaleQuality', 'hd')"
                            style="padding: 1rem; border-radius: 0.75rem; border: 2px solid {{ $upscaleQuality === 'hd' ? 'rgba(6,182,212,0.6)' : 'var(--vw-border)' }}; background: {{ $upscaleQuality === 'hd' ? 'rgba(6,182,212,0.1)' : 'rgba(0,0,0,0.03)' }}; cursor: pointer; text-align: center;">
                        <div style="font-size: 1.5rem; margin-bottom: 0.35rem;">🖼️</div>
                        <div style="color: var(--vw-text); font-weight: 600; font-size: 1rem;">HD</div>
                        <div style="color: var(--vw-text-secondary); font-size: 0.75rem; margin-top: 0.25rem;">1920 × 1080</div>
                    ...
                    <button type="button"
                            ...
                            style="padding: 1rem; border-radius: 0.75rem; border: 2px solid {{ $upscaleQuality === '4k' ? 'rgba(251,191,36,0.6)' : 'var(--vw-border)' }}; background: {{ $upscaleQuality === '4k' ? 'rgba(251,191,36,0.1)' : 'rgba(0,0,0,0.03)' }}; cursor: pointer; text-align: center;">
                        <div style="font-size: 1.5rem; margin-bottom: 0.35rem;">✨</div>
                        <div style="color: var(--vw-text); font-weight: 600; font-size: 1rem;">4K</div>
                        <div style="color: var(--vw-text-secondary); font-size: 0.75rem; margin-top: 0.25rem;">3840 × 2160</div>
            ...
            <div style="background: rgba(251,191,36,0.08); border: 1px solid rgba(251,191,36,0.2); border-radius: 0.5rem; padding: 0.75rem; margin-bottom: 1.25rem;">
                <div style="display: flex; align-items: start; gap: 0.5rem;">
                    ...
                    <div style="color: var(--vw-text-secondary); font-size: 0.8rem; line-height: 1.4;">
