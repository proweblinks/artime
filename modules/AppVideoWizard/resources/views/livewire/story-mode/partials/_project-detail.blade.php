    ...
    <div class="d-flex gap-4 position-relative" style="max-width: 900px; width: 100%; max-height: 90vh; padding: 20px;">
        ...
        <div class="flex-grow-1 d-flex flex-column" style="min-width: 260px; max-height: 80vh; overflow-y: auto;">
            <div class="p-4" style="background: #ffffff; border: 1px solid #eef1f5; border-radius: 12px; box-shadow: var(--at-glass, 0 4px 24px rgba(0,0,0,0.06));">
                ...
                {{-- Style --}}
                <div class="mb-4">
                    <label class="small d-block mb-2" style="color: var(--at-text-muted, #94a0b8);">{{ __('Style') }}</label>
                    <div class="d-flex align-items-center gap-2">
                        @if($project->style)
                            ...
                            @else
                                <div style="width: 32px; height: 32px; border-radius: 6px; background: #f5f7fa;"
                                     class="d-flex align-items-center justify-content-center">
                                    <i class="fa-light fa-palette" style="color: var(--at-text-muted, #94a0b8);"></i>
                                </div>
                            @endif
                            <span class="fw-semibold" style="font-size: 0.9rem; color: var(--at-text, #1a1a2e);">{{ $project->style->name }}</span>
                        @else
                            <span style="color: var(--at-text-muted, #94a0b8);">{{ __('Custom') }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="d-flex flex-column gap-2">
                    @if($project->style_id)
                        <button wire:click="useThisStyle({{ $project->id }})" type="button"
                                class="btn w-100" style="background: #f5f7fa; color: var(--at-text, #1a1a2e); border-radius: 10px;">
